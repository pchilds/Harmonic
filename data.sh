...
echo ""
echo "void prf(GtkPrintOperation *prto, GtkPrintContext *ctex, int page_nr)"
echo "{"
echo "	cairo_t *cr=gtk_print_context_get_cairo_context(ctex);"
echo "	gchar *str;"
echo ""
echo "	if (page_nr) return;"
...
echo "}"
echo ""
echo "void prb(GtkPrintOperation *prto, GtkPrintContext *ctex, int page_nr)"
echo "{"
echo "	gtk_print_operation_set_current_page(prto, 0);"
echo "	gtk_print_operation_set_has_selection(prto, FALSE);"
echo "}"
echo ""
echo "void prt(GtkWidget *widget, gpointer data)"
echo "{"
echo "	GtkPrintOperation *prto;"
echo "	GtkPageSetup *prps;"
...
echo "	g_signal_connect(prto, \"draw_page\", G_CALLBACK(prf), NULL);"
...
echo "	gchar *contents=NULL, *fin=NULL, *str;"
...
echo "			}"
echo "			flags|=PROC_OPN;"
echo "			x=g_array_new(FALSE, FALSE, sizeof(gdouble));"
echo "			y=g_array_new(FALSE, FALSE, sizeof(gdouble));"
echo "			if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(anosa))) {k=2; lc=-1;}"
echo "			else if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(sws))) {k=0; lc=-1;}"
echo "			else {k=0; lc=0;}"
...
echo "					strat=g_strsplit_set(strary[k], \"\\t\", 0);"
echo "					lcl=g_ascii_strtod(g_strstrip(strat[0]), NULL);"
echo "					g_array_append_val(x, lcl);"
echo "					if (lc==0)"
echo "					{"
...
echo "				g_free(str);"
echo "				plt=GTK_PLOT_LINEAR(plot1);"
echo "				xi=g_array_index(x, gdouble, 0);"
echo "				xf=g_array_index(x, gdouble, (lc-1));"
