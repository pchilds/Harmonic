...
echo ""
echo "#include \"batc.h\""
echo "GtkWidget *dialog, *adb, *spin, *cft;"
...
echo "{"
echo "	GtkWidget *wwfile, *lbl, *btt;"
echo "	GtkAdjustment *adj;"
echo "	GList *chld;"
echo "	GSList *fls, *list;"
echo "	gint rw, j;"
echo "	gint ps;"
echo "	gdouble val;"
echo ""
echo "	wwfile=gtk_file_chooser_dialog_new(_(\"Select Data File\"), GTK_WINDOW(dialog), GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);"
...
echo "			list=(fls->next);"
echo "			lbl=gtk_label_new((gchar*) (fls->data));"
...
echo "		while (fls)"
echo "		{"
echo "			btt=gtk_button_new_from_stock(GTK_STOCK_GO_DOWN);"
echo "			g_signal_connect(G_OBJECT(btt), \"clicked\", G_CALLBACK(tsw), GINT_TO_POINTER(ps));"
echo "			gtk_table_attach(GTK_TABLE(cft), btt, 0, 1, rw-3, rw-2, GTK_FILL|GTK_SHRINK|GTK_EXPAND, GTK_FILL|GTK_SHRINK|GTK_EXPAND, 2, 2);"
echo "			gtk_widget_show(btt);"
echo "			confb=g_slist_prepend(confb, (gpointer) btt);"
echo "			btt=gtk_button_new_from_stock(GTK_STOCK_GO_UP);"
echo "			g_signal_connect(G_OBJECT(btt), \"clicked\", G_CALLBACK(tsw), GINT_TO_POINTER(ps));"
echo "			gtk_table_attach(GTK_TABLE(cft), btt, 0, 1, rw-2, rw-1, GTK_FILL|GTK_SHRINK|GTK_EXPAND, GTK_FILL|GTK_SHRINK|GTK_EXPAND, 2, 2);"
echo "			gtk_widget_show(btt);"
echo "			confb=g_slist_prepend(confb, (gpointer) btt);"
echo "			btt=gtk_button_new_from_stock(GTK_STOCK_DELETE);"
echo "			g_signal_connect(G_OBJECT(btt), \"clicked\", G_CALLBACK(tdl), GINT_TO_POINTER(ps));"
...
echo "	gint dr, dx, dx2, j, jdim, jdimx, k, kdim, kib, l, lc, lcib, m, mx, n, sal, sr, sp, trc, zp;"
echo "	GList *iter1, *iter2;"
echo "	GSList *list;"
echo "	GtkAdjustment *adj, *adj2;"
echo "	GtkPlotLinear *plt;"
echo "	GtkPlotPolar *plt2;"
echo "	GtkWidget *cont, *content, *hbox, *label, *scroll, *vsc, *wfile;"
echo "	struct confrow *row;"
echo ""
...
echo "			label=gtk_label_new(_(\"File:\"));"
echo "			gtk_widget_show(label);"
echo "			gtk_table_attach(GTK_TABLE(cft), label, 1, 2, 0, 1, GTK_FILL|GTK_SHRINK|GTK_EXPAND, GTK_FILL|GTK_SHRINK|GTK_EXPAND, 2, 2);"
echo "			label=gtk_label_new(_(\"Measurand\nValue:\"));"
echo "			gtk_widget_show(label);"
echo "			gtk_table_attach(GTK_TABLE(cft), label, 2, 3, 0, 1, GTK_FILL|GTK_SHRINK|GTK_EXPAND, GTK_FILL|GTK_SHRINK|GTK_EXPAND, 2, 2);"
